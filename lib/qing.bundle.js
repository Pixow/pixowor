import { Injectable, InjectionToken, NgModuleRef, ɵConsole, NgZone, Type, Injector, NgModuleFactoryLoader, Compiler, Optional, SystemJsNgModuleLoader, Version, NgModuleFactory, ɵisObservable, ɵisPromise, Component, NgModule, Inject, PLATFORM_ID } from '@angular/core';
...
import { Location, ɵgetDOM, ɵparseCookieValue, DOCUMENT } from '@angular/common';
...
var ElectronService = /** @class */ (function () {
    ...
    Object.defineProperty(ElectronService.prototype, "appPath", {
        get: function () {
...
var LocalStorageService = /** @class */ (function () {
    function LocalStorageService() {
        LocalStorageService_1.instance = this;
    }
...
function combineLatest() {
    ...
    if (observables.length === 1 && isArray$4(observables[0])) {
...
function forEach$1(map, callback) {
...
class UrlSegmentGroup {
    ...
    children) {
        ...
        this.parent = null;
        forEach$1(children, (v, k) => v.parent = this);
    }
...
function mapChildrenIntoArray(segment, fn) {
    let res = [];
    forEach$1(segment.children, (child, childOutlet) => {
...
function serializeSegment(segment, root) {
    ...
    if (root) {
        const primary = segment.children[PRIMARY_OUTLET] ?
            ...
            '';
        const children = [];
        forEach$1(segment.children, (v, k) => {
            if (k !== PRIMARY_OUTLET) {
...
function decodeQuery(s) {
    return decode$1(s.replace(/\+/g, '%20'));
}
...
class UrlParser {
    ...
    parseSegment() {
        ...
        this.capture(path);
        return new UrlSegment(decode$1(path), this.parseMatrixParams());
    }
    parseMatrixParams() {
        const params = {};
        while (this.consumeOptional(';')) {
...
function tree(oldSegmentGroup, newSegmentGroup, urlTree, queryParams, fragment) {
    let qp = {};
    if (queryParams) {
        forEach$1(queryParams, (value, name) => {
...
function replaceSegment(current, oldSegment, newSegment) {
    const children = {};
    forEach$1(current.children, (c, outletName) => {
        if (c === oldSegment) {
...
function computeNavigation(commands) {
    ...
    const res = commands.reduce((res, cmd, cmdIdx) => {
        if (typeof cmd === 'object' && cmd != null) {
            if (cmd.outlets) {
                const outlets = {};
                forEach$1(cmd.outlets, (commands, name) => {
...
function updateSegmentGroupChildren(segmentGroup, startIndex, commands) {
    if (commands.length === 0) {
        ...
        const children = {};
        forEach$1(outlets, (commands, outlet) => {
        ...
        forEach$1(segmentGroup.children, (child, childOutlet) => {
...
class ActivateRoutes {
    ...
    deactivateChildRoutes(futureNode, currNode, contexts) {
        ...
        forEach$1(children, (v, childName) => {
    ...
    deactivateRouteAndOutlet(route, parentContexts) {
        ...
        if (context) {
            ...
            forEach$1(children, (v, k) => this.deactivateRouteAndItsChildren(v, contexts));
            if (context.outlet) {
...
function isCanLoad(guard) {
    return guard && isFunction$2(guard.canLoad);
}
function isCanActivate(guard) {
    return guard && isFunction$2(guard.canActivate);
}
function isCanActivateChild(guard) {
    return guard && isFunction$2(guard.canActivateChild);
}
function isCanDeactivate(guard) {
    return guard && isFunction$2(guard.canDeactivate);
}
...
class ApplyRedirects {
    ...
    createQueryParams(redirectToParams, actualParams) {
        const res = {};
        forEach$1(redirectToParams, (v, k) => {
    ...
    createSegmentGroup(redirectTo, group, segments, posParams) {
        ...
        let children = {};
        forEach$1(group.children, (child, name) => {
...
function getChildRouteGuards(futureNode, currNode, contexts, futurePath, checks = {
    ...
    forEach$1(prevChildren, (v, k) => deactivateRouteAndItsChildren(v, contexts.getContext(k), checks));
    return checks;
}
...
function deactivateRouteAndItsChildren(route, context, checks) {
    const children = nodeChildrenAsMap(route);
    const r = route.value;
    forEach$1(children, (node, childName) => {
        if (!r.component) {
 ...
 * @docsNotRequired
 */
new InjectionToken('ROUTER_FORROOT_GUARD');
 ...
 * @publicApi
 */
new InjectionToken('Router Initializer');
...
// @dynamic
var QingWebApiService = /** @class */ (function () {
    function QingWebApiService(router) {
        this.router = router;
        // if (WorkbenchConfig.environment === "LOCAL") {
        this._sdk = new QingWebApiSdk(Environment.Alpha);
        // }
        QingWebApiService_1.instance = this;
    }
    QingWebApiService_1 = QingWebApiService;
    Object.defineProperty(QingWebApiService.prototype, "sdk", {
        get: function () {
            return this._sdk;
        },
        enumerable: false,
    ...
    QingWebApiService.prototype.setInterceptors = function (user) {
        var _this = this;
        this._sdk.setRequestInterceptor(function (config) { return __awaiter(_this, void 0, void 0, function () {
        ...
        this._sdk.setResponseInterceptor(function (response) {
            return response;
            ...
            return __generator(this, function (_a) {
                ...
                if (error.response.status === 403 || (error.response.status === 401 && !originalRequest._retry)) {
                    ...
                    this._sdk.auth
                        ...
                        .then(function (res) {
                        _this._sdk.setToken(res.data.token);
                        return _this._sdk.req(originalRequest);
                    })
    ...
    var QingWebApiService_1;
    QingWebApiService = QingWebApiService_1 = __decorate([
        Injectable(),
        __metadata("design:paramtypes", [Router])
    ], QingWebApiService);
    return QingWebApiService;
}());
...
var WORKBENCH_PUZZLE_BLOCK;
(function (WORKBENCH_PUZZLE_BLOCK) {
    WORKBENCH_PUZZLE_BLOCK["WORKBENCH_ACTIVITYBAR"] = "WorkbenchActivitybar";
    WORKBENCH_PUZZLE_BLOCK["WORKBENCH_EXPLORER"] = "WorkbenchExplorer";
})(WORKBENCH_PUZZLE_BLOCK || (WORKBENCH_PUZZLE_BLOCK = {}));
var Puzzle = /** @class */ (function () {
    function Puzzle(contextService) {
        this.contextService = contextService;
        this.slots = new Map();
        this.plugins = new Map();
    }
    Puzzle.prototype.registPuzzleSlot = function (id, container) {
        var block = new PuzzleSlot(id, container);
        if (!this.slots.has(block.id)) {
            this.slots.set(block.id, block);
        }
    };
    Puzzle.prototype.getPuzzleSlot = function (id) {
        return this.slots.get(id);
    };
    Puzzle.prototype.getPlugin = function (pluginName) {
        return this.plugins.get(pluginName);
    };
    Puzzle.prototype.use = function (plugin) {
        ...
        if (plugin.contributes && typeof plugin.contributes === "string") {
            var slot = this.slots.get(plugin.contributes);
        }
        else {
            for (var _i = 0, _a = Object.keys(plugin.contributes); _i < _a.length; _i++) {
                var slotName = _a[_i];
                var config = plugin.contributes[slotName];
                var slot = this.slots.get(slotName);
                if (slot) {
...
var PuzzleSlot = /** @class */ (function () {
    function PuzzleSlot(id, container) {
        this.config = {};
        this.items = [];
        this.id = id;
        this.container = container;
    }
    PuzzleSlot.prototype.injectConfig = function (config) {
        var _this = this;
        // this.config = Object.assign(this.config, config);
        this.items = this.items.concat(config);
        setTimeout(function () {
            _this.container.items = _this.items;
        });
    };
    return PuzzleSlot;
}());
...
var F__PixelPai_qingProject_qing_node_modules_querystring = createCommonjsModule(function (module, exports) {
...
Url.prototype.parse = function(url, parseQueryString, slashesDenoteHost) {
  ...
  if (!slashesDenoteHost && url.split('#').length === 1) {
    ...
    if (simplePath) {
      ...
      if (simplePath[2]) {
        ...
        if (parseQueryString) {
          this.query = F__PixelPai_qingProject_qing_node_modules_querystring.parse(this.search.substr(1));
        } else {
  ...
  if (qm !== -1) {
    ...
    if (parseQueryString) {
      this.query = F__PixelPai_qingProject_qing_node_modules_querystring.parse(this.query);
    }
...
Url.prototype.format = function() {
  ...
  if (this.query &&
    ...
    query = F__PixelPai_qingProject_qing_node_modules_querystring.stringify(this.query);
  }
...
var logo = join(__dirname, "assets/images/logo.ico");
var BaseConfig = {
    production: false,
    environment: "LOCAL",
    logo: logo,
...
var WorkbenchConfig = assign$1(BaseConfig, {
    production: false,
    environment: "LOCAL",
...
/** @class */ ((function () {
    ...
    Object.defineProperty(User.prototype, "avatarUrl", {
        get: function () {
            if (this.avatar) {
                return resolve(WorkbenchConfig.WEB_RESOURCE_URI, this.avatar);
            }
            else {
                return null;
            }
...
var utils$2 = { stringify: stringify$2, stripBom };
...
const { stringify: stringify$3, stripBom: stripBom$1 } = utils$2;
...
async function _readFile (file, options = {}) {
  ...
  let data = await universalify$1.fromCallback(fs.readFile)(file, options);
...
const readFile = universalify$1.fromPromise(_readFile);
...
async function _writeFile (file, obj, options = {}) {
  ...
  await universalify$1.fromCallback(fs.writeFile)(file, str, options);
}

const writeFile = universalify$1.fromPromise(_writeFile);
...
const { stringify: stringify$4 } = utils$2;
...
const { stringify: stringify$5 } = utils$2;
...
/** @class */ ((function () {
    ...
    Object.defineProperty(Game.prototype, "gameCover", {
        get: function () {
            return this.cover ? resolve(WorkbenchConfig.WEB_RESOURCE_URI, this.cover) : "./assets/images/game_thumb.png";
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(Game.prototype, "isExists", {
        get: function () {
            return lib$1.pathExistsSync(this.gameFolder);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(Game.prototype, "lastGameVersion", {
        get: function () {
            var _this = this;
            if (this.lastVersion) {
                return this.pastVersions.find(function (item) { return item.version === _this.lastVersion; });
            }
            else {
                return last$2(this.pastVersions);
            }
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(Game.prototype, "gameFolder", {
        get: function () {
            return join(this._storePath, this.owner.username + "/game/" + this._id);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(Game.prototype, "_storePath", {
        get: function () {
            return join(electron.remote.app.getPath("userData"), "QingData");
        },
        enumerable: false,
        configurable: true
    });
    Game.prototype.getGameZipUri = function (version) {
        return resolve(WorkbenchConfig.WEB_RESOURCE_URI, this.owner.username + "/game/" + this._id + "/" + version + "/" + this._id + ".zip");
    };
    return Game;
}()));

var DynamicInjector = /** @class */ (function () {
    function DynamicInjector(_parentInjector, _additionalTokens) {
        this._parentInjector = _parentInjector;
        this._additionalTokens = _additionalTokens;
    }
    DynamicInjector.prototype.get = function (token, notFoundValue, flags) {
...
class HttpRequest {
    ...
    serializeBody() {
        ...
        if (isArrayBuffer$1(this.body) || isBlob$1(this.body) || isFormData$1(this.body) ||
...
var ContextService = /** @class */ (function () {
    function ContextService(http, compiler, electronService, injector) {
        this.http = http;
        this.compiler = compiler;
        this.electronService = electronService;
        this.injector = injector;
        this.activityItem$ = new BehaviorSubject(null);
        this.pluginComponentFactories = new Map();
        this.pluginComponents = new Map();
        console.log("ContextService init");
    }
    ContextService.prototype.getPluginConfigs = function () {
        return this.http.get("plugins-repo/plugins.config.json");
        // fsa.readJson(this.electronService.appPath);
    };
    ContextService.prototype.initial = function () {
        console.log("Plugin use workbench context success.");
    };
    ContextService.prototype.createPuzzle = function () {
        this._puzzle = new Puzzle(this);
    };
    Object.defineProperty(ContextService.prototype, "puzzle", {
        get: function () {
            return this._puzzle;
        },
        enumerable: false,
    ...
    Object.defineProperty(ContextService.prototype, "sdk", {
    ...
    Object.defineProperty(ContextService.prototype, "localStorage", {
        get: function () {
    ...
    ContextService.prototype.setActivitybar = function (item) {
        this.activityItem$.next(item);
    };
    ContextService.prototype.getComponentFactory = function (pluginName) {
        return this.pluginComponentFactories.get(pluginName);
    };
    ContextService.prototype.getComponent = function (pluginName) {
        return this.pluginComponents.get(pluginName);
    };
    ContextService.prototype.triggerRender = function (pluginName) {
        var plugin = this.puzzle.getPlugin(pluginName);
        var slot = this.puzzle.getPuzzleSlot(plugin.contributes);
        var factory = this.pluginComponentFactories.get(pluginName);
        console.log("🚀 ~ file: context.service.ts ~ line 56 ~ ContextService ~ factory", factory);
        if (factory) {
            slot.container.createComponent(factory);
        }
    };
    ContextService.prototype.loadModule = function (pluginName, path) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            path()
                ...
                try {
                    ...
                    var moduleRef = moduleFactory.create(new DynamicInjector(_this.injector, map));
                    var componentFactory = moduleRef.componentFactoryResolver.resolveComponentFactory(moduleRef.instance.componentType);
                    _this.pluginComponentFactories.set(pluginName, componentFactory);
                    _this.pluginComponents.set(pluginName, moduleRef.instance.componentType);
                    resolve(true);
                    // do something with the module...
                }
                catch (err) {
